...
- `A3_2`: impone di selezionare solo le modalità di imaging esplicitamente citate per *questo* paziente, senza farsi condizionare dall'elenco generico di modalità nel contesto Brighton. Nota aggiunta successivamente: il testo Brighton a volte raggruppa più tecniche in un'unica descrizione combinata (es. "ultrasound with and without doppler"), mentre lo schema le elenca come opzioni separate — l'hint ora chiarisce esplicitamente di basarsi sulla tecnica specifica nominata nell'evidenza del paziente, non su come il riferimento le raggruppa o le ordina.
...
- `F`: chiarisce che la sezione richiede "Sì" solo se la diagnosi è riportata *senza* alcun dettaglio clinico di supporto. Nota aggiunta successivamente: "dettagli clinici" va giudicato dalla presenza di un reperto specifico (anatomia, misura, risultato), non dal fatto che il nome del test/procedura compaia nel frammento di evidenza estratto da Agent 1 — l'estrazione può tagliare via il nome del test pur lasciando il reperto, e l'assenza del nome da sola non implica "nessun dettaglio".
