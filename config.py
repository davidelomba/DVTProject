...
SECTION_HINTS = {
    ...
    "A3_2": (
        ...
        "- ultrasound that assesses blood flow (color Doppler, duplex, echo-color-Doppler, "
        "flow or velocity study, absent/present flow signal) -> 'Doppler/Duplex Ultrasound'\n"
        "- ultrasound that only tests whether the vein collapses under the probe -> "
    ...
    "X": (
        ...
        "'ALTERNATIVE' MEANS ALTERNATIVE TO DVT, not alternative to the diagnosis the record settles on. "
        "When the record explicitly names a competing condition as the cause of the presentation, that "
        "condition IS the alternative diagnosis and you MUST select 'An alternative diagnosis was found "
        "that explained the acute illness' -- this holds even when the record presents it as the final, "
        "confirmed diagnosis rather than as one hypothesis among several. "
        "Select 'No alternative diagnosis was found to explain the acute illness' when the record names "
        "no such competing condition, including when it diagnoses DVT itself."
    ),
}
