...
The myocarditis arm has no hints, no cross-section rules and no guideline
context, because all three are written for the DVT form or wait on a paper that
has not arrived. Comparing it with the reference would therefore measure the
transfer plus the removal of those three. This run removes the same three from
the DVT corpus, so the two arms differ in the questionnaire and not in the
configuration.
